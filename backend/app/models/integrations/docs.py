...
class DocsCreateRequest(BaseModel):
    ...
    content: Optional[str] = Field(
        description="Content to insert into the document, if any"
    )


class DocsFilterRequest(BaseModel):
    id: str = Field(description="Document id to filter documents with")
...
class DocsUpdateRequest(DocsFilterRequest):
    updated_content: str = Field(
        description="Updated content to replace the document with"
    )
